...
class TraceAnalyzer:
      def __init__(self,user_input,final_answer,tools,mode,mutant_type):
          self.tools = tools
          self.mode = mode
      ...
      def analyze_trace(self, trace):
          ...
          fallback_keywords = [
                "unable", "not able", "can't", "cannot", "fail to", "unknown",
          ...
          if 'agent stopped' not in self.final_answer.lower():
                  ...
                  if any(kw in final_answer_lower for kw in fallback_keywords):
                      pass
                  elif self.mode=='mock' and self.mutant_type=='real/simulated' and not all(resp.strip() == "Complete Response" for resp in complete_tool_response):
